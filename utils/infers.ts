...
// implement a Replace type for one occurence
type Replace<S extends string, From extends string, To extends string> =
  From extends '' ? S :
  S extends `${infer Prefix}${From}${infer Suffix}` ? `${Prefix}${To}${Suffix}` :
  S
type ReplacedString = Replace<'foobar', 'bar', 'foo'>

//implement a ReplaceAll for all occurences
type ReplaceAll<S extends string, From extends string, To extends string> =
  From extends '' ? S :
  S extends `${infer Prefix}${From}${infer Suffix}` ? `${ReplaceAll<Prefix, From, To>}${To}${ReplaceAll<Suffix, From, To>}` :
  S
type ReplacedAllString = ReplaceAll<'foobarbar', 'bar', 'foo'> 

// implement a StartsWith type for one occurence
type StartsWith<T extends string, U extends string> = T extends `${U}${infer rest}` ? true : false;
type StartsWithString = StartsWith<'foobar', 'foo'>